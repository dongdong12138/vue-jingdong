<template>

  <div class="wrapper">

    <div class="position">
      <span class="iconfont position__icon">&#xe619;</span>
      北京理工大学国防科技园2号楼10层
      <span class="iconfont position_notice">&#xe60b;</span>
    </div>

    <div class="search">
      <span class="iconfont">&#xe62d;</span>
      <span class="search__text">山姆会员商店优惠商品</span>
    </div>

    <div class="banner">
      <img src="http://www.dell-lee.com/imgs/vue3/banner.jpg" alt="banner" class="banner__img" />
    </div>

    <div class="icons">
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/超市.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">超市便利</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/菜市场.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">菜市场</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/水果店.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">水果店</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/鲜花.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">鲜花绿植</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/医药健康.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">医药健康</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/家居.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">家居时尚</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/蛋糕.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">烘培蛋糕</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/签到.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">签到</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/大牌免运.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">大牌免运</p>
      </div>
      <div class="icons__item">
        <img src="http://www.dell-lee.com/imgs/vue3/红包.png" alt="icon" class="icons__item__img" />
        <p class="icons__item__desc">红包套餐</p>
      </div>
    </div>

    <div class="gap"></div>

    <div class="nearby">
      <h3 class="nearby__title">附近店铺</h3>
      <div class="nearby__item">
        <img src="http://www.dell-lee.com/imgs/vue3/near.png" alt="near" class="nearby__item__img" />
        <div class="nearby__content">
          <div class="nearby__content__title">沃尔玛</div>
          <div class="nearby__content__tags">
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
          </div>
          <p class="nearby__content__highlight">VIP尊享满89元减4元运费券（每月3张）</p>
        </div>
      </div>
      <div class="nearby__item">
        <img src="http://www.dell-lee.com/imgs/vue3/near.png" alt="near" class="nearby__item__img" />
        <div class="nearby__content">
          <div class="nearby__content__title">沃尔玛</div>
          <div class="nearby__content__tags">
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
          </div>
          <p class="nearby__content__highlight">VIP尊享满89元减4元运费券（每月3张）</p>
        </div>
      </div>
      <div class="nearby__item">
        <img src="http://www.dell-lee.com/imgs/vue3/near.png" alt="near" class="nearby__item__img" />
        <div class="nearby__content">
          <div class="nearby__content__title">沃尔玛</div>
          <div class="nearby__content__tags">
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
          </div>
          <p class="nearby__content__highlight">VIP尊享满89元减4元运费券（每月3张）</p>
        </div>
      </div>
      <div class="nearby__item">
        <img src="http://www.dell-lee.com/imgs/vue3/near.png" alt="near" class="nearby__item__img" />
        <div class="nearby__content">
          <div class="nearby__content__title">沃尔玛</div>
          <div class="nearby__content__tags">
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
          </div>
          <p class="nearby__content__highlight">VIP尊享满89元减4元运费券（每月3张）</p>
        </div>
      </div>
      <div class="nearby__item">
        <img src="http://www.dell-lee.com/imgs/vue3/near.png" alt="near" class="nearby__item__img" />
        <div class="nearby__content">
          <div class="nearby__content__title">沃尔玛</div>
          <div class="nearby__content__tags">
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
          </div>
          <p class="nearby__content__highlight">VIP尊享满89元减4元运费券（每月3张）</p>
        </div>
      </div>
      <div class="nearby__item">
        <img src="http://www.dell-lee.com/imgs/vue3/near.png" alt="near" class="nearby__item__img" />
        <div class="nearby__content">
          <div class="nearby__content__title">沃尔玛</div>
          <div class="nearby__content__tags">
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
            <span class="nearby__content__tag">月售1万+</span>
          </div>
          <p class="nearby__content__highlight">VIP尊享满89元减4元运费券（每月3张）</p>
        </div>
      </div>
    </div>

  </div>

  <div class="docker">
    <div class="docker__item docker__item--active">
      <div class="iconfont">&#xe6f3;</div>
      <div class="docker__title">首页</div>
    </div>
    <div class="docker__item">
      <div class="iconfont">&#xe7e5;</div>
      <div class="docker__title">购物车</div>
    </div>
    <div class="docker__item">
      <div class="iconfont">&#xe61e;</div>
      <div class="docker__title">订单</div>
    </div>
    <div class="docker__item">
      <div class="iconfont">&#xe660;</div>
      <div class="docker__title">我的</div>
    </div>
  </div>

</template>

<style lang="scss">
@import './style/viriables.scss';
@import './style/mixins.scss';

.wrapper {
  overflow-y: auto;
  padding: 0 .18rem .1rem .18rem;
  position: absolute; left: 0; top: 0; bottom: .5rem; right: 0;
}

.position {
  color: $content-fontcolor; font-size: .16rem;
  line-height: .22rem;
  padding: .16rem .24rem .16rem 0;
  position: relative;
  @include ellipsis;
  .position__icon {
    font-size: .2rem;
    position: relative; top: .01rem;
  }
  .position_notice {
    font-size: .2rem;
    position: absolute; right: 0; top: .17rem;
  }
}

.search {
  color: #B7B7B7;
  background: #F5F5F5;
  border-radius: .16rem;
  line-height: .32rem;
  margin-bottom: .12rem;
  .iconfont { display: inline-block; font-size: .16rem; padding: 0 .08rem 0 .16rem; }
  &__text { display: inline-block; font-size: .14rem; }
}

.banner {
  height: 0;
  overflow: hidden;
  padding-bottom: 25.4%;
  &__img { width: 100%; }
}

.icons {
  display: flex; flex-wrap: wrap;
  margin-top: .16rem;
  &__item {
    width: 20%;
    &__img {
      display: block;
      width: .4rem; height: .4rem;
      margin: 0 auto;
    }
    &__desc {
      color: $content-fontcolor;
      text-align: center;
      margin: .06rem 0 .16rem 0;
    }
  }
}

.gap {
  height: .1rem;
  background: $content-bgColor;
  margin: 0 -.18rem;
}

.nearby {
  &__title {
    color: $content-fontcolor; font-size: .18rem; font-weight: normal;
    margin: .16rem 0 .02rem 0;
  }
  &__item {
    display: flex;
    padding-top: .12rem;
    &__img {
      width: .56rem; height: .56rem;
      margin-right: .16rem;
    }
  }
  &__content {
    flex: 1;
    border-bottom: 1px solid $content-bgColor;
    padding-bottom: .12rem;
    &__title {
      color: $content-fontcolor; font-size: .16rem;
      line-height: .22rem;
    }
    &__tags {
      color: $content-fontcolor; font-size: .13rem;
      line-height: .18rem;
      margin-top: .08rem;
    }
    &__tag {
      margin-right: .16rem;
    }
    &__highlight {
      color: #E93B3B; font-size: .13rem;
      line-height: .18rem;
      margin: .08rem 0 0 0;
    }
  }
}

.docker {
  box-sizing: border-box;
  color: $content-fontcolor;
  width: 100%; height: .49rem;
  border-top: .01rem solid $content-bgColor;
  padding: 0 .18rem;
  position: absolute; left: 0; bottom: 0;
  display: flex;
  &__item {
    flex: 1;
    text-align: center;
    .iconfont { font-size: .18rem; margin: .07rem 0 .02rem 0; }
    &--active { color: #1FA4FC; }
  }
  &__title { font-size: .2rem; transform: scale(.5, .5); transform-origin: center top; }
}
</style>
